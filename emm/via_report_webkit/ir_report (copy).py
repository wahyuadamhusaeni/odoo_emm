...
class via_form_template_webkit_parser(WebKitParser):
    ...
    def generate_pdf(self, comm_path, report_xml, header, footer, html_list, webkit_header=False):
        ...
        for a in command:
            command_baru = str(command_baru) + " " + str(a)

        os.system(command_baru)

        # subprocess.call([command_baru], stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        # subprocess.run(command)

        # try:
        #     # status = subprocess.call(command, stderr=subprocess.PIPE)  # ignore stderr
        #     status = subprocess.Popen(command_baru, shell=True,stdin=None,stdout=subprocess.PIPE,stderr=subprocess.PIPE)  # ignore stderr
        # #     print(status)
        #     if status:
