...
class wizard(wizard):
    ...
    def print_report(self, cr, uid, form, context=None):
        ...
        if len(form.customer_ids) == 0:
            ...
            where_clause = "AND cust.id IN (%s)" % (','.join([str(x.id) for x in form.customer_ids]))

        order_clause = 'customer, so_no, product_code, so_qty DESC, so_amt DESC'
