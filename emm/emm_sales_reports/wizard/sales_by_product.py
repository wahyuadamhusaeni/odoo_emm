...
class wizard(wizard):
    def onchange_company_ids(cr, uid, ids, com_ids, context=None):
        if len(com_ids) == 0:
            return {
                'domain': {'prod_ids': [('company_id', '=', False), ]},
                'value': {'prod_ids': False},
            }
        return {
            'domain': {'prod_ids': [('company_id', 'in', com_ids[0][2]), ('sale_ok', '=', True)]},
            'value': {'prod_ids': False},
        }
    ...
    def print_report(self, cr, uid, form, context=None):
        ...
        if len(form.prod_ids) == 0:
            ...
            where_clause = "AND prod.id IN (%s)" % (','.join([str(x.id) for x in form.prod_ids]))

        order_clause = 'product, product_code, customer, so_date, so_no, so_qty DESC, so_amt DESC'
        ...
        form.add_marshalled_data('SQL_PARAMS', sql_for_sales_report())
