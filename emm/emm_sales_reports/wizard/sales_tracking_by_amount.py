...
class wizard(wizard):
    ...
    def onchange_company_ids(cr, uid, ids, com_ids, context=None):
        ...
        if len(com_ids) == 0:
            return {
                'domain': {'customer_ids': [('company_id','=',False),]},
                'value': {'customer_ids': False},
            }
        return {
            'domain': {'customer_ids': [('company_id','in',com_ids[0][2]),]},
            'value': {'customer_ids': False},
        }
    ...
    def print_report(self, cr, uid, form, context=None):
        ...
        form.add_marshalled_data('SQL_PARAMS', sales_tracking_sql())
